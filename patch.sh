...
# Taken from https://github.com/JeffreyCA/Apollo-ImprovedCustomApi/blob/main/patch.sh
...
if [ ! -f "$INPUT_IPA" ] && [ ! -d "$INPUT_IPA" ]; then
...
rm -rf extract_temp
if [ -d "$INPUT_IPA" ]; then
    echo "Input is already a directory, copying..."
    mkdir extract_temp
    if [ -d "$INPUT_IPA/Payload" ]; then
        cp -r "$INPUT_IPA/Payload" extract_temp/
    elif [ $(basename "$INPUT_IPA") == "Payload" ]; then
        cp -r "$INPUT_IPA" extract_temp/
    else
        echo "Error: Directory must contain Payload/ or be named Payload"
        exit 1
    fi
else
